...
export class SenseService {
	private defaultVoiceName = 'Google 普通话（中国大陆）';
	...
	private createSpeaker() {
		...
		if (this.window[key as any]) {
			console.log('speaker found');
			this.speaker = this.window[key as any];
			this.getVoices();
			return;
		}

		this.speaker = new SpeechSynthesisUtterance();
		this.window[key as any] = this.speaker;
	...
	private createListener() {
		...
		if (this.window[key as any]) {
			...
			this.listener = this.window[key as any];
			this.startListening();
			return;
		}
		...
		this.window[key as any] = this.listener;
